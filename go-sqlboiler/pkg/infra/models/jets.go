...
var JetWhere = struct {
	...
	ID:      whereHelperint{field: "\"app_default\".\"jets\".\"id\""},
	PilotID: whereHelperint{field: "\"app_default\".\"jets\".\"pilot_id\""},
	Age:     whereHelperint{field: "\"app_default\".\"jets\".\"age\""},
	Name:    whereHelperstring{field: "\"app_default\".\"jets\".\"name\""},
	Color:   whereHelperstring{field: "\"app_default\".\"jets\".\"color\""},
}
...
func (jetL) LoadPilot(ctx context.Context, e boil.ContextExecutor, singular bool, maybeJet interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`app_default.pilots.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func Jets(mods ...qm.QueryMod) jetQuery {
	mods = append(mods, qm.From("\"app_default\".\"jets\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"app_default\".\"jets\".*"})
	}
...
func FindJet(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Jet, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"app_default\".\"jets\" where \"id\"=$1", sel,
	)
	...
	if err = jetObj.doAfterSelectHooks(ctx, exec); err != nil {
		return jetObj, err
	}
...
func (o *Jet) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"app_default\".\"jets\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"app_default\".\"jets\" %sDEFAULT VALUES%s"
		}
...
func (o *Jet) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"app_default\".\"jets\" SET %s WHERE %s",
...
func (o JetSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"app_default\".\"jets\" SET %s WHERE %s",
...
func (o *Jet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"app_default\".\"jets\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Jet) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"app_default\".\"jets\" WHERE \"id\"=$1"
...
func (o JetSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"app_default\".\"jets\" WHERE " +
...
func (o *JetSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"app_default\".\"jets\".* FROM \"app_default\".\"jets\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, jetPrimaryKeyColumns, len(*o))
...
func JetExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"app_default\".\"jets\" where \"id\"=$1 limit 1)"
...
// Exists checks if the Jet row exists.
func (o *Jet) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return JetExists(ctx, exec, o.ID)
}
